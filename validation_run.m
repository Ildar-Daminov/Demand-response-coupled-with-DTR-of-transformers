function [PUL_optim,Pflex_max,Eflex_max,theta_h_optim,theta_0_optim]=validation_run(LOADprofile,AMB,SOCflex_init,SOCflex_end,ageing_constraint_on)


%% Load Simulation profiles
%--------------------------
if length(LOADprofile)==1440 || rem(length(LOADprofile),1440)==0
    [LOADprofile,~]=create_load(LOADprofile,60);% Convert transformer loadings
elseif length(AMB)==24|| rem(length(AMB),24)==0
    AMB=PUL_to_1min(AMB,60);
elseif rem(length(LOADprofile),1440)==0&&rem(length(AMB),24)==0
    
else
    error('check length of LOADprofile and/or AMB in initial_solution_approximation ')
end
...
% SOCflex_init=0.5;  %battery initial SOC in %
% SOCflex_end=0.5;  %battery initial SOC in %
...
Pflex_max=sdpvar(1,1); % Maximal power of flexibility, kW      
Eflex_max=sdpvar(1,1); % Maximal energy of flexibiity, kWh
...
if SOCflex_init==1 && SOCflex_end==0 % energy shedding mode
    Constraints=[Constraints ; Pflex >= 0 ];  
elseif SOCflex_init==0.5 && SOCflex_end==0.5 % energy shifting mode
    Constraints=[Constraints ; Pflex >= -Pflex_max ];
end
...
if ageing_constraint_on==1 % if ageing contraint is activated 
...
end % end of "if ageing_constraint_on==1"

%% Solve & Extract Results
%-------------------------

% Select a solver linprog
ops = sdpsettings('solver', 'linprog','savesolveroutput',1 ,'verbose',1);

% Solve the optimization problem
sol=optimize(Constraints,Objective, ops);

% Find the number of variables and constraints
...
figure()
subplot(2,3,1), stairs(0:(HorizonD-1),[ Pload Ptr ]/Nominal_rating, 'linewidth', 2)
xlabel('Time (h)')
ylabel('Loading (pu)')
legend('No Flex', 'With Flex','Location','northwest')
...
xlabel('Time (h)')
ylabel('T (deg)')
legend('No Flex', 'With Flex', 'Limit','Location','northwest')
